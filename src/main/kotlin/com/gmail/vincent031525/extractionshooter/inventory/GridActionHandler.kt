...
object GridActionHandler {
        ...
        if (button == 1) { // Right-click
            if (carried.isEmpty) {
                if (targetStack.item is MagazineItem) {
                    val data = MagazineItem.getMagazineData(targetStack) ?: return false
                ...
                if (targetStack.item is MagazineItem && carried.item is AmmoItem) {
                    val data = MagazineItem.getMagazineData(targetStack) ?: return false
                    val stats = (targetStack.item as MagazineItem).getMagazineStats() ?: return false
            ...
            if (!carried.isEmpty && ItemStack.isSameItemSameComponents(targetStack, carried)) {
                return targetStack.count < targetStack.maxStackSize
            }
        }
    ...
    fun interact(
        ...
        if (button == 1) { // Right-click (SECONDARY)
            if (workingCarried.isEmpty) {
                if (targetStack.item is MagazineItem) {
        ...
        } else if (button == 0) { // Left-click (PRIMARY)
            if (!workingCarried.isEmpty && ItemStack.isSameItemSameComponents(targetStack, workingCarried)) {
                val spaceLeft = targetStack.maxStackSize - targetStack.count
                if (spaceLeft > 0) {
